...
function handleCommmunicationRoundReponse(socket) {
  socket.on('participant-round-response', function(roundResult) {
    const senderClient = findParticipantbyId(clients, socket.id);
    // need to add all sort of checks to avoid attackers to inject undesired messages
...
function startRound() {
  clients = Participant.prototype.resetParticipants(clients)
  broadcastMessage('start-communication-round');
  ++currentRoundIndex;
  rounds[currentRoundIndex] = new Round(currentRoundIndex+1);
}
...
function endRound(roundResult) {
  broadcastMessage('voting-round-result', roundResult);
...
function endCommunicationRound(roundResult) {
  broadcastMessage('communication-round-result', roundResult);
  rounds[currentRoundIndex].completed = true;
  rounds[currentRoundIndex].roundResult = roundResult;
  --messageLength;
  if(messageLength === 0) {
      broadcastMessage('show-communicated-message');
      // reset everything
      // collapse current session
      // start next session with some delay
      clients = Participant.prototype.resetParticipants(clients);
      clients = addPendingParticipants(clients, pendingClients);
      pendingClients.length = 0;
      clients = removeMultipleParticipants(clients, clientsToBeRemoved);
...
function generateAndExchangeKeys(adjacencies) {
  ...
  adjacencies.forEach(function(adjacency, index, array){
    Promise.try(function() {
      ...
      unicastMessage(adjacency.participant1.id, 'round-key-generated', [keyName1, keyValue]);
      unicastMessage(adjacency.participant2.id, 'round-key-generated', [keyName2, keyValue]);
