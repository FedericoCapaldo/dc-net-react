...
function handleParticipantResponse(socket) {
  socket.on('participant-response', function(clientResultValue) {
    findClientbyId(clients, socket.id).roundResult = clientResultValue;
...
function generateAndExchangeKeys(adjacencies) {
  ...
  adjacencies.forEach(function(adjacency, index, array){
    Promise.try(function() {
      ...
      // could this be a single multicast event?
      unicastMessageWithType(adjacency.participant1.id, 'key-generated', keyName1, number);
      unicastMessageWithType(adjacency.participant2.id, 'key-generated', keyName2, number);
...
function computeAndDistributeRoundResult() {
  ...
  final = final % 2;
  broadcastMessage("round-result", final);
  roundInProgress = false;
  // should check if there are participants waiting
  resetRoundResultValues();
  setTimeout(function() {
    beginCommunication();
  }, 3000)
}
...
function resetRoundResultValues() {
  clients.map(client => client.roundResult = -1);
}
