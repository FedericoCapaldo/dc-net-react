...
let messageSender ;
let messageLength ;
let rejectedParticipantsVotingRound = [] ;
let roundNumber = 0;
  ...
  socket.on('participant-voting-response', function(participantMessage) {
    ...
    if(participantMessage === expectedMessage) {
      ...
      if (rounds[currentRoundIndex].didSomeoneSendMessage) {
        ...
        rejectedParticipantsVotingRound = [...rejectedParticipantsVotingRound, sender];
      } else {
        sender.roundMessage = participantMessage;
...
function broadcastEvent(event, message = '') {
  participants.forEach(function(connectedParticipant, index, array) {
...
function communicateIfAnyRejected() {
  if(rejectedParticipantsVotingRound.length) {
    rejectedParticipantsVotingRound.forEach(function(rejectedParticipant, index, arr) {
      unicastEvent(rejectedParticipant.id, 'message-rejected');
    })
    rejectedParticipantsVotingRound.length = 0;
  }
}

function addPendingParticipants() {
  participants = mergeArrays(participants, pendingParticipants);
  pendingParticipants.forEach(function (pendingParticipant, index, array) {
...
// debugging
function debugPrintParticipantsFromArray() {
  participants.forEach(function(c, index, array) {
    console.log(c.name + "\n");
