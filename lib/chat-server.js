...
function handleParticipantResponse(socket) {
  socket.on('participant-response', function(clientResultValue) {
    findClientbyId(clients, socket.id).roundMessage = clientResultValue;
...
function resetRoundMessagesValues() {
  clients.map(client => client.roundMessage = -1);
}
